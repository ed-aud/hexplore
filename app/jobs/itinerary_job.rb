class ItineraryJob < ApplicationJob
  ...
  def itinerary_formatted_for_openai
    ...
    system_text = "You are an assistant to help plan a day out around Shoreditch High Street, and you should provide an itinerary for a day full of activities based on the parks, pubs, museums, and other points of interest within 1 kilometre of Shoreditch High Street station. To format the message, remove any reference to latitude/longitude. Remove all ** or ##. Start with one introductory paragraph followed by one bullet for the morning, one for lunchtime, one for the afternoon and one for the evening and a final paragraph for tips and a reference that the user can save the hexagon to their hive. One line space between each section"
